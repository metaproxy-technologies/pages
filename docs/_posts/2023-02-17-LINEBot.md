...
graph LR;
  ...
  User -- text --> LINEBot --> CloudFunctions --> DALL-E

  subgraph Frontend;
    LINEBot
  end
  subgraph Image Generator;
    CloudFunctions
    DALL-E
  end
...
- If you are planning to use AudioMessage in LINE, it's better to choose pyhon3.9 as LINE utilizes AAC(m4a) format as Audio so pydub and ffmpeg is required but ffmpeg do not run in python3.11 and python 3.10 env.
